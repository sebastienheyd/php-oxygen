...
if (!function_exists('get_called_class'))
{
    ...
    function get_called_class()
    {
        ...
        foreach ($bt as $k => $v)
        {
            ...
            // special case.
            if ($v['function'] === 'call_user_func' || $v['function'] === 'call_user_func_array')
                return $v['args'][0][0];
            ...
            if (preg_match('/([a-zA-Z0-9\_]+)::' . $v['function'] . '/', $lineContent, $matches) && isset($matches[1]))
            {
                switch ($matches[1])
                {
                    ...
                    case 'parent':
                        for ($i = $line; $i >= 0; $i--)
                        {
                            if (preg_match('/class[\s]+([a-zA-Z0-9\_]+)+?/si', trim($scriptContent[$i]), $matches))
                                return $matches[1];
                        }
                        break;
...
if (!function_exists('get_module_file'))
{
    ...
    function get_module_file($module, $filePath, $check = true)
    {
        ...
        if ($check && !is_file(MODULES_DIR . DS . $module . DS . $filePath))
            return false;
...
if (!function_exists('set_header'))
{
    ...
    function set_header($code = 200, $text = '')
    {
        ...
        if ($code === '' || !is_numeric($code))
            trigger_error('Status codes must be numeric', E_USER_ERROR);

        if (isset($status[$code]) && $text == '')
            $text = $status[$code];

        if ($text === '')
            trigger_error('No status text available.  Please check your status code number or supply your own message text.', E_USER_ERROR);
